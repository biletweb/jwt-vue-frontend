  ...
  <div class="flex justify-center">
    <div class="w-4/12 rounded-2xl border bg-white p-5">
      <div class="flex flex-col gap-5">
        ...
        <button v-else @click="signIn" class="mt-5 w-full rounded-lg bg-green-500 p-2 text-white hover:bg-green-600">Sign up</button>
      </div>
    </div>
  </div>
</template>
...
async function signIn() {
  await authStore.auth({ name: name.value, email: email.value, password: password.value, password_confirmation: password_confirmation.value }, 'signup')
}
</script>
